...
type positionRouters = { [K in Origami.Server.Position]: Router };
...
export class Server {
  ...
  constructor(options?: Origami.ConfigServer, store?: any) {
    ...
    if (process.env.PORT) {
      this._options.port = parseInt(process.env.PORT);
    }
  ...
  public useRouter(router: Route, requireNamedMiddleware: boolean = true) {
    Object.entries(this._positionRouters).forEach(([p, pr]) => {
      router.routers[p as Origami.Server.Position].forEach(
        ({ handlers, method }: RouterListItem) => {
          // Convert all the named handlers (EG: router.use('auth')) into request handlers
          const mappedNamedHandlers = handlers
            .map((h) => {
            ...
            errorWrapped.unshift((req, res, next) => {
              log(
                ...
                res.locals.content.hasContent
                  ? colors.green('has content')
                  : colors.red('no content')
              );
              next();
            });
          }
  ...
  public async plugin(
    ...
    if (Boolean(settings)) {
      ...
      try {
        plugin = await requireLib(name, p, [
          '@origami/plugin-',
          'origami-plugin-'
        ]);
      } catch (e) {
      ...
      if (typeof plugin !== 'function') {
        ...
        if (plugin && typeof plugin.default === 'function') {
          ...
          throw new err.ErrorServerPluginNotFunction(name);
        }
      }
  ...
  public resource(name: string, options: ResourceOptions) {
    if (!this.store) {
      throw new err.ErrorServerStoreNotConfigured();
    }
  ...
  public namedMiddleware(name: string, handler: Origami.Server.RequestHandler) {
    if (this._namedMiddleware[name]) {
      throw new err.ErrorServerExistingNamedMiddleware(name);
    }
  ...
  public getMiddlewareForPath(p: string): Function[] {
    const mw = Object.values(this._positionRouters).reduce(
      (_mw, r) => {
        _mw.push(
          ...(r.stack
  ...
  public static(
    p: string,
    prefix?: string,
    position: Origami.Server.Position = 'post-render'
  ) {
  ...
  private async _setupStatic() {
    ...
    if (this._options.static === undefined || this._options.static === true) {
      try {
        (await statAsync(
          path.join(process.cwd(), DEFAULT_STATIC)
        )).isDirectory();
